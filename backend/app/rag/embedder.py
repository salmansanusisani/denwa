"""Embeds chunk text into fixed-size vectors.

Provider strategy:
  1. fastembed (ONNX Runtime) — real semantic embeddings, runs locally on CPU,
     no API key. Model BAAI/bge-small-en-v1.5 (384-dim, ~30MB) is downloaded
     once to ~/.cache/fastembed. Override with the EMBEDDING_MODEL env var.
  2. Deterministic offline hashing fallback (numpy only, no model download) if
     fastembed is unavailable — keeps retrieval working everywhere.
