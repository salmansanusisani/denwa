"""Turns retrieved chunks into a CALL-E task string + resultSchema.

This is the contract the Backend worker calls: ``build_task(company_id,
likely_topic)`` returns ``{"task", "result_schema", "content"}``.
...
Prompt-safety rules baked into the task string:
  ...
  - When it can't answer confidently, it says so and offers a human callback.
...
from app.rag.retriever import retrieve_for_topic, retrieve_company_context
...
def _build(company_id: int, chunks: list[str]) -> dict:
    ...
    return {"task": task, "result_schema": RESULT_SCHEMA, "content": content}
