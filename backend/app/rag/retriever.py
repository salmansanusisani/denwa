"""Given a likely topic (the customer's exact question is unknown before the
call), return the most relevant chunks for that company.

The real question is unknown pre-call, so ``retrieve_company_context()`` also
supports a set of likely topics and returns the union of matches.
"""
