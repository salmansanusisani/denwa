...
Handles two common layouts:
  - Structured FAQ/policy text with "Q:" / "A:" (or "Question:" / "Answer:"):
    one chunk per Q&A pair so the answer stays with its question.
...
def _split_qa_pairs(text: str) -> list[str]:
    """Chunk structured Q/A text into pairs: a question line plus everything up
    to the next question line (answers, follow-ups and sub-notes stay with Q)."""
    pairs: list[str] = []
    current: list[str] = []
...
def chunk_text(raw_text: str) -> list[str]:
    """Split raw text into retrieval-sized chunks (plain text pieces)."""
    ...
    for segment in segments:
        for window in _sliding_windows(segment):
            if window not in pieces:
